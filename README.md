...
`dragActivationConstraints` lets consumers tune when pointer drags become active. The prop is optional. When omitted, the tree uses dnd-kit's default pointer sensor behavior.

If `mouse`, `touch`, or `pen` is omitted from the object, that input type also uses dnd-kit's default behavior. Pass a constraint object to override a pointer type. `pen` falls back to the `mouse` constraint when `pen` is omitted and `mouse` is provided.
...
| `dragActivationConstraints`  | `{ mouse?: SortableTreeActivationConstraint \| null; touch?: SortableTreeActivationConstraint \| null; pen?: SortableTreeActivationConstraint \| null; }` | dnd-kit defaults | Experimental drag activation constraints for pointer-based sensors. Omit the prop or an input type to use dnd-kit's default behavior, or pass a constraint object to override it. |
